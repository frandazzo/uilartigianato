...
/**
 * Created by antoniolovicario on 23/02/15.
 */
public class FenealDateUtils {
    ...
    public static String getDescriptionByMonthCode(int monthNumber) {
        switch (monthNumber) {
    ...
    public static double getDifferencesInHours(Date start, Date end) {
        ...
        double hours = (endDateInMills - startDateInMills) / (1000 * 60 * 60); //trasforma tutto in ore
        return hours;
    ...
    public static int getMonthNumber(Date date) {
        /*
        Ottiene il numero del mese (da 1 a 12) per la data passata
         */
        Calendar cal = getCalendarInstance(date);
        return cal.get(Calendar.MONTH) + 1; //in java i mesi partono da 0
    }

    public static int getDifferencesInMonths(Date start, Date end) {
        Calendar sDate = getCalendarInstance(start);
        Calendar eDate = getCalendarInstance(end);
        sDate.setTime(start);
        eDate.setTime(end);
    ...
    public static Date getWeekStartDate(Date date) {
        ...
        for (int day : calendarDays(false)) {
    ...
    public static Date getNextWeekDay(Date date) {
        ...
        while (!found) {
            ...
            if (calendar.get(Calendar.WEEK_OF_MONTH) != dayToFound.get(Calendar.WEEK_OF_MONTH) || calendar.get(Calendar.MONTH) != dayToFound.get(Calendar.MONTH)) {
                found = true;
            } else {
                continue;
            }
    ...
    public static List<Date> getDaysOfWeek(Date date) {
        Calendar day = FenealDateUtils.getCalendarInstance(date);
        List<Date> dayOfWeeks = new ArrayList<>();
        for (int dayNumber : FenealDateUtils.calendarDays(false)) {

            Calendar weekDay = FenealDateUtils.getCalendarInstance(date);
    ...
    public static List<Date> getDaysOfMonth(Date date) {
        Date firstDayOfMonth = FenealDateUtils.getMonthStartDate(date);
        Calendar first = FenealDateUtils.getCalendarInstance(firstDayOfMonth);
        Calendar day = FenealDateUtils.getCalendarInstance(firstDayOfMonth);
    ...
    public static Date getDayBeforeEndOfMonth(Date date, int dayBeforeEnd) {
        ...
        calendar.add(Calendar.DAY_OF_MONTH, -1 * dayBeforeEnd);
        return calendar.getTime();
    }
    ...
    public static List<Date> getAllMonthWeekStartsFromDates(List<Date> dates) {
        ...
        for (Date date: dates) {
            weekStarts.put(date.getTime(), FenealDateUtils.getWeekStartDate(date));
        }
    ...
    public static List<Date> getDaysBetweenRange(Date startDate, Date endDate) {
        ...
        Calendar first = FenealDateUtils.getCalendarInstance(startDate);
        Calendar last = FenealDateUtils.getCalendarInstance(endDate);

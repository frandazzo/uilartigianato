...
(function($, window, document, undefined) {
   ...
   $.fn.adminpanel = function(options) {
      ...
      //controllo l'ultimo utente loggato se l'utente attuale è diverso dal precedente annullo il local storage
      var lastUser = localStorage.getItem("lastUser");
      var currentUser =  window.appcontext.loggeduser;
      if (lastUser != currentUser){
         localStorage.clear();
         localStorage.setItem("lastUser", window.appcontext.loggeduser);
      }



      // HTML5 LocalStorage Keys
      //metto un segnaposto nelle chiavi del localstorage per salvare separatamente i 
      //dati per ogni contesto //dashboard, worker e azienda
      if (!options.context)
          throw "Contesto delle opzioni mancante!";
      var settingsKey = options.context + '_panel-settings_' + location.pathname;
      var positionsKey = options.context + '_panel-positions_' + location.pathname;
      ...
      var methods = {
         ...
         applySettings: function(options) {
            ...
            $.each(parseSettings, function(i, e) {
               ...
               $.each(e, function(i, e) {
                  ...
                  if (panelHidden === 1) {
                     Target.addClass('panel-hidden').hide().empty();
                  }
               });
            });
         },
         ...
         controlHandlers: function(e) {
            ...
            var panelRemove = function() {
               ...
               if (bootbox.confirm) {
                  bootbox.confirm("Sicuro?", function(e) {
            ...
            //questa feature è stata implementata nel controllo widget nel widgetManager

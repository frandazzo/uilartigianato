...
define([
    ...
    var Contatore = widgetManager.DashboardWidget.extend({
        ctor:function(widgetName, widgetParams){
            ...
            if (!this.anno){
                var annoDefault = $(".contatore-segnalazioni-container").find('input[name="hidden-year"]').val();
                this.anno = annoDefault;
            }

            if (!this.mese){
                var meseDefault = $(".contatore-segnalazioni-container").find('input[name="hidden-month"]').val();
        ...
        prepareWidget: function(){
            ...
            service.set("url",BASE + "widget/real/contatoreSegnalazioni/getData");
            ...
            service.on("load",function(resp){
                ...
                var pie1 = $('#high-pie-contatore-segnalazioni');

                if (pie1.length) {

                    // Pie Chart1
                    $('#high-pie-contatore-segnalazioni').highcharts({
                        chart:{
                            type:'pie',
                            height: 300,
                        ...
                        credits: false,
                        title: {
                            text: 'Segnalazioni per categoria'
                        },
                        subtitle: {
                ...
                $('#high-pie-contatore-segnalazioni').children().addClass("full-width-important");
        ...
        renderHtmlForm : function(editPanel, params){
            ...
            var monthSelect = $(".contatore-segnalazioni-container").find('div.param-fields-container').find('select[name="month"]').clone();
            ...
            var yearSelect = $(".contatore-segnalazioni-container").find('div.param-fields-container').find('select[name="year"]').clone();

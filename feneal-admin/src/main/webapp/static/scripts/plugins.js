...
define(["framework/core"], function(core) {
            ...
            var methods = {
                ...
                add: function (item) {
                    ...
                    if (!options.allowDuplicates)
                        if ($(selectionContainer).find(".selection[data-value='" + fixValue(item.value) + "']").length > 0)
                            return;
    ...
    /* categorysearchableInput */
    (function ($) {
        $.fn.categorySearchableInput = function (opts) {
            var main = null;
            var buttonsContainer = null;
            var keywordTextbox = null;
            var resultsDiv = null;
            ...
            var mainText = null;
            //combo box per i 
            var keywordCombo = null;
            var keyWordComboValues = null;


            function prefilterCombo(value){
                if (!value)
                    return;
                $(resultsDiv).selectList("clear");
                $(keywordTextbox).val("");

                //adesso prendo dalla lista dei risultati tutti tutti quelli  con questo valore
                var list = keyWordComboValues.filter(function(elem){
                   return elem.value == value;
                });


                loadCombo(keywordCombo, list);
            ...
            function loadDataForCombo(combo, url){

                $.post(url, {

                }, function (values) {
                    keyWordComboValues = values.value;
                    loadCombo(combo, keyWordComboValues);
                }, "json");
            ...
            function createDialog() {
                ...
                keywordCombo = _E("select")
                    .addClass("keywordcombo")
                    .addClass("form-control");

                var categoryUrl = options.categoryServiceUrl;
                loadDataForCombo(keywordCombo, categoryUrl);
            ...
            function doSearch() {
                ...
                var url = options.serviceUrl;
                var keyword = $(keywordTextbox).val();
                var comboSelectedValue = $(keywordCombo).val();

                $.post(url, {
                    keyword: keyword,
                    category: comboSelectedValue
                }, function (response) {
                    $(resultsDiv).selectList("clear");
                    if (response && !response.error) {
                        $(resultsDiv).selectList("addAll", response.value);
            ...
            function initSingle() {
                ...
                $(main)
                    ...
                    .keyup(function (e) {
                        if (e.which == 13) {
                            //quando apro la modale imposto un filtro sui valori possibili
                            //della select
                            prefilterCombo($(main).attr("data-prefilter-value"));
            ...
            var methods = {
                init: function (opts) {
                    if (!opts.serviceUrl)
                        throw "Service url not specified";
                    if (!opts.categoryServiceUrl)
                        throw "CategoryService url not specified";
                    if (!opts.name)
                        throw "Name not specified";
    ...
    (function ($) {
        $.remoteModalDialog = function (opts) {
            ...
            function checkContainer() {
                if ($("#remoteModalDialogContainer").length == 0) {
                    $("body").append(
    ...
    (function ($) {
        ...
        Breadcrumbs.prototype = {
            ...
            add: function (breadcrumb) {
                ...
                var size = $(this.element).find("li").length;
                if(size > 0) {

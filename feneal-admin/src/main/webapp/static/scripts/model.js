...
define(["framework/core", "framework/ui"], function(core, ui) {
    ...
    var GridService = core.AObject.extend({
        ...
        normalize: function(loadRequest){

            //devo ciclare tra tutti gli oggetti della load request.filters e verificare se ci sono proprietà con lo stesso nome che provvedero' ad inserire in un array
            ...
            for (var prop in loadRequest.filters){
                ...
                var propName =  loadRequest.filters[prop].property;
                if (!propertyBuffer[propName]){
                    propertyBuffer[propName] = new Array();
                    propertyBuffer[propName].push(loadRequest.filters[prop]);
                }else{
                    propertyBuffer[propName].push(loadRequest.filters[prop]);
                }
            ...
            //adesso faccio l'inverso: ricostruisco l'oggetto filters a partire dal buffer
            var filters = new Array();
            for(var propName in propertyBuffer){

                if (propertyBuffer[propName].length == 1) //se ce n'è solo una ne riprendo la property
                {
                    filters.push(propertyBuffer[propName][0]);
                }else{
                    //adesso devo ricostruire un oggetto che ha la property e il type come indicato nel primo oggetto e come valori
                    //un array di valori contenti i valori degli oggetti

                    var type = propertyBuffer[propName][0].type;

                    //adesso recupero i valori degli oggetti
                    var values = new Array();
                    $.each(propertyBuffer[propName], function(index, elem){
        ...
        load: function(opts) {
          ...
          //  alert('bruno è gaio');
            ...
            $.ajax({
                type: this.method,
                url: this.url,
                data: $.extend(this.data, { loadRequest: JSON.stringify(self.normalize(this.loadRequest) )}),
                dataType: "json",
                success: function(response) {
                    self.element = null;
